...
function mountGoogleReviews(el) {
  ...
  fetch('/.netlify/functions/google-reviews?widget_key=' + encodeURIComponent(widgetKey))
    ...
    .catch(function() {
      ...
      if (wrap) wrap.innerHTML = 'Could not load reviews.';
    });
}
...
function renderWidget(el, data) {
  ...
  var cardsPerPage = (typeof window !== 'undefined' && window.innerWidth <= 768) ? 1 : 4;
  var page = 0;
  var totalPages = Math.ceil(reviews.length / cardsPerPage);

  var dotsHTML = '';
  for (var i = 0; i < totalPages; i++) {
    dotsHTML += '<button class="sf-gr-dot' + (i === 0 ? ' active' : '') + '" data-idx="' + i + '"></button>';
  }

  wrap.innerHTML = '<div class="sf-gr-header">' +
  ...
  readmoreBtns.forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      var card = btn.closest('.sf-gr-card');
      var cards = wrap.querySelectorAll('.sf-gr-card');
      var idx = Array.prototype.indexOf.call(cards, card);
      body.textContent = reviews[idx].text;
      btn.remove();
    });
  });
  ...
  wrap.querySelector('.sf-gr-prev').addEventListener('click', function() { goTo(page - 1); });
  wrap.querySelector('.sf-gr-next').addEventListener('click', function() { goTo(page + 1); });
